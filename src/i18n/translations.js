export const translations = {
    ru: {
        heroTitle: "UPuti — экономьте на поездках, зарабатывайте на дороге.",

        heroDesc:
            "UPuti — сервис попутных поездок нового поколения.\n" +
            "Путешествуйте как пассажир или зарабатывайте как водитель на дороге, по которой уже едете.",

        aboutText:
            "UPuti помогает водителям и пассажирам быстро находить друг друга, экономить время и делить расходы на дорогу.",

        driverTitle: "Для водителей",
        driverDesc:
            "Опубликуйте поездку и находите попутчиков. Пассажиры могут забронировать место или предложить свою цену.",

        passengerTitle: "Для пассажиров",
        passengerDesc:
            "Находите поездки по городу и между городами, бронируйте место и путешествуйте дешевле и удобнее.",

        tripsTitle: "Поездки по городу и между городами",
        tripsIntro: "Uputi подходит для любых поездок:",
        tripsList: [
            "ежедневные поездки по городу",
            "междугородние поездки",
            "поиск попутчиков на удобное время",
        ],
        benefitsTitle: "Почему UPuti",
        benefitsList: [
            "безопасные поездки",
            "удобный поиск поездок",
            "бронирование в несколько кликов",
            "возможность предложить свою цену",
            "экономия времени и денег",
        ],

        ctaTitle: "Скачайте приложение UPuti и находите попутчиков за секунды.\n" +
            "Начните экономить на поездках уже сегодня.",

        download: "Скачать приложение",

        androidDownload: "Скачать для Android",
        iosDownload: "Скачать для iOS",
        support: "Связаться с нами",
        supportTitle: "Напишите в поддержку",
        supportDesc: "Опишите кратко проблему или вопрос. Сообщение откроется в Telegram чате @uputi_support.",
        supportPlaceholder: "Например: не приходит код, не получается создать поездку…",
        supportSend: "Написать в Telegram",
        statsUsers: "3 000+ пользователей по всему Узбекистану",
        statsTrips: "1 000+ поездок в каждом городе каждый день по всему Узбекистану",
        storeDownloads: "3 000+ скачиваний UPuti в Google Play и App Store",
        playNoticeTitle: "🚧 Приложение пока недоступно в Google Play",
        playNoticeP1: "Мы уже работаем над этим, и скоро приложение появится в магазине.",
        playNoticeP2Prefix: "Но вы пока можете скачать и установить приложение напрямую",
        playNoticeLinkText: "👉 здесь",
        playNoticeSafety: "🔒 Это безопасно — файл размещён только на нашем официальном сайте и не содержит вирусов.",
        playNoticeThanks: "Спасибо за доверие 🙌",
        socialFollow: "Мы в соцсетях",
        telegramLabel: "Telegram канал UPuti",
        instagramLabel: "Instagram UPuti",
    },

    uz: {
        heroTitle: "UPuti — yo'l kirani tejab qoling, yo'lda pul ishlang",

        heroDesc:
            "UPuti —  taksi emas, aksincha: shahar ichida va shaharlar orasida safar qilish uchun poputchiklar servisi. Kimdir yo‘lovchi, kimdir esa o‘zi ketayotgan yo‘lida daromad qiladigan haydovchi.",

        aboutText:
            "Bu haydovchi va yo‘lovchilarga bir-birini tez topishga, vaqtni tejashga va yo‘l xarajatlarini bo‘lishishga yordam beradi.",

        driverTitle: "Haydovchilar uchun",
        driverDesc:
            "Safaringizni e’lon qiling va yo‘lovchilarni toping. Yo‘lovchilar joy bron qilishi yoki o‘z narxini taklif qilishi mumkin.",

        passengerTitle: "Yo‘lovchilar uchun",
        passengerDesc:
            "Shahar ichida va shaharlar orasida safarlarni toping, joy bron qiling va arzonroq sayohat qiling.",

        tripsTitle: "Shahar va shaharlararo safarlar",
        tripsIntro: "Uputi har qanday safar uchun mos:",
        tripsList: [
            "kundalik shahar safarlari",
            "shaharlararo safarlar",
            "qulay vaqtda yo‘lovchi qidirish",
        ],
        benefitsTitle: "Nega UPuti",
        benefitsList: [
            "qulay safar qidiruv",
            "bir necha marta bosishda bron qilish",
            "o‘z narxingizni taklif qilish imkoniyati",
            "vaqtni tejash",
            "pulni tejash",
        ],

        ctaTitle: "Bugunoq foydalanishni boshlang",
        ctaDesc:
            "UPuti ilovasini yuklab oling va bir necha soniyada yo‘lovchi toping.",

        download: "Ilovani yuklab olish",

        androidDownload: "Android uchun yuklab olish",
        iosDownload: "iOS uchun yuklab olish",
        support: "Biz bilan bog'lanish",
        supportTitle: "Yordam xizmati bilan yozishish",
        supportDesc: "Muammo yoki savolingizni qisqacha yozing. Xabar @uputi_support Telegram chatida ochiladi.",
        supportPlaceholder: "Masalan: kod kelmayapti, safar yaratib bo‘lmayapti…",
        supportSend: "Telegram orqali yuborish",
        statsUsers: "3 000+ foydalanuvchi",
        statsTrips: "Har bir shaharda kuniga 1 000+ safar",
        storeDownloads: "3 000+ UPuti yuklab olishlar butun O'zbekistonda",
        playNoticeTitle: "🚧 Ilova hozircha Google Play’da mavjud emas",
        playNoticeP1: "Biz bu ustida allaqachon ishlayapmiz va tez orada ilova Google Play’da paydo bo‘ladi.",
        playNoticeP2Prefix: "Lekin siz ilovani to‘g‘ridan-to‘g‘ri yuklab olib o‘rnatishingiz mumkin",
        playNoticeLinkText: "👉 bu yerda",
        playNoticeSafety: "🔒 Bu xavfsiz — fayl faqat rasmiy saytimizda joylangan va viruslardan himoyalangan.",
        playNoticeThanks: "Ishonchingiz uchun rahmat 🙌",
        socialFollow: "Biz ijtimoiy tarmoqlarda",
        telegramLabel: "UPuti Telegram kanali",
        instagramLabel: "UPuti Instagram",
    },
};